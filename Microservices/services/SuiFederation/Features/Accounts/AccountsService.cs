...
public class AccountsService : IService
{
    ...
    public async Task<List<Vault>> GetVaultsByPrefixAndBalance(string prefix, string coinModule, long minBalance = 0)
    {
        var vaults= await _vaultCollection.GetVaultsByPrefix(prefix);
        return vaults
            .Where(vault => vault.VaultBalance?.FirstOrDefault(balance => balance.CoinModule == coinModule)?.Balance >= minBalance)
            .ToList();
    }

    public async Task SetCoinBalance(string accountName, string coinName, long balance)
    {
        await _vaultCollection.UpsertCoinBalance(accountName, new VaultBalance(coinName, balance));
    }
}