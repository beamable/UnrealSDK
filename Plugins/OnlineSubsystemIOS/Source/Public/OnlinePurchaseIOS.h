...
public:
	...
	/**
	 * Method used internally by FStoreKitPurchaseProxy to handle finalized transactions. Not meant to be called by user code
	 */
	void OnRemovedTransactions(const TArray<FOnlinePurchaseTransactionIOS>& Transactions);
...
private:
	...
	/** Transactions in the process of being finalized */
	TArray<FKnownTransaction> FinalizingTransactions;

	/** Reference to the parent subsystem */
	FOnlineSubsystemIOS* Subsystem;
	
	/** Delegate fired when a transaction is finalized */
	using FOnTransactionFinalized = TMulticastDelegate<void(const FOnlinePurchaseTransactionIOS&)>;
	FOnTransactionFinalized OnTransactionFinalized;
};
