...
TSharedPtr<FMonitoredProcess> UBeamCliUnityReleaseSharedCodeCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("unity release-shared-code");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliUnityReleaseSharedCode Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliUnityReleaseSharedCode Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if (Bag.FromJson(MessageJson))
			{
				...
				if(ReceivedStreamType.Equals(StreamType))
				{
					...
					UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliUnityReleaseSharedCode Command - Message Received: %s"), *MessageJson);
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliUnityReleaseSharedCode Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
