...
#include "BeamCliContentHistorySyncChangelistCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliContentHistorySyncChangelistStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	UContentHistoryChangelistStreamData* Changelist = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeUObject<UContentHistoryChangelistStreamData*>("Changelist", Changelist, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeUObject<UContentHistoryChangelistStreamData*>("Changelist", Bag, Changelist, OuterOwner);	
	}
};
...
/**
 Description:
  [INTERNAL] Syncs a changelist for a given manifest UID. If already cached locally, touches the file to trigger a watching history command

Usage:
  Beamable.Tools content history sync-changelist [options]
...
Options:
  ...
  --manifest-uid <manifest-uid>              The manifest UID for the changelist to sync
 ...
 */
UCLASS()
class UBeamCliContentHistorySyncChangelistCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliContentHistorySyncChangelistStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliContentHistorySyncChangelistStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
