...
#include "Subsystems/CLI/Autogen/StreamData/OrgRealmDataStreamData.h"
#include "BeamCliOrgRealmsCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliOrgRealmsStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	FString Cid = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<UOrgRealmDataStreamData*> VisibleRealms = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("CustomerAlias"), CustomerAlias, Serializer);
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("Cid"), Cid, Serializer);
		UBeamJsonUtils::SerializeArray<UOrgRealmDataStreamData*>(TEXT("VisibleRealms"), VisibleRealms, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("CustomerAlias"), Bag, CustomerAlias);
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("Cid"), Bag, Cid);
		UBeamJsonUtils::DeserializeArray<UOrgRealmDataStreamData*>(TEXT("VisibleRealms"), Bag, VisibleRealms, OuterOwner);	
	}
};
...
/**
 Description:
  Fetch the current list of realms

Usage:
  Beamable.Tools org realms [options]

Options:
  --include-archived                         Includes archived realms in the list of realms returned
 ...
 */
UCLASS()
class UBeamCliOrgRealmsCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliOrgRealmsStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliOrgRealmsStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
