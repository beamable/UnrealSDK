...
#include "BeamCliProjectWriteSettingCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliProjectWriteSettingStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools project write-setting <beamoId> [options]

Arguments:
  <beamoId>  The BeamoId to write the settings for

Options:
  -s, --skip, --skip-build                   If options are modified, the project needs to be re-built so that the embedded resource is available on the next execution of the service. However, this build operation may be skipped when this option is set
  -k, --key <key>                            The keys of the settings. The count must match the count of the --value options
  -d, --delete-key <delete-key>              The keys of the settings to be deleted. If the key is also set in a --key option, the key will not be deleted
  -v, --value <value>                        The values of the settings. The count must match the count of the --key options
 ...
 */
UCLASS()
class UBeamCliProjectWriteSettingCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliProjectWriteSettingStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliProjectWriteSettingStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
