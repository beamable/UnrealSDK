...
#include "BeamCliTokenNewGuestCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliTokenNewGuestStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools token new-guest [options]

Options:
  -k, --key <key>                            The keys for initProperties. The count must match the count of the --value options
  -v, --value <value>                        The values for the initProperties. The count must match the count of the --key options
 ...
 */
UCLASS()
class UBeamCliTokenNewGuestCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliTokenNewGuestStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliTokenNewGuestStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
