...
/**
 Description:
  Restore a dotnet project

Usage:
  Beamable.Tools unity restore [options]

Options:
  --csproj <csproj>                          The path to the dotnet csproj path
 ...
 */
UCLASS()
class UBeamCliUnityRestoreCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliUnityRestoreStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliUnityRestoreStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
