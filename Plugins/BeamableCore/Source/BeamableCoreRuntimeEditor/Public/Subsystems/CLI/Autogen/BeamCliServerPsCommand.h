...
#include "Subsystems/CLI/Autogen/StreamData/ServerDescriptorStreamData.h"
#include "BeamCliServerPsCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliServerPsStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	TArray<UServerDescriptorStreamData*> Servers = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<UServerDescriptorStreamData*>(TEXT("servers"), Servers, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeArray<UServerDescriptorStreamData*>(TEXT("servers"), Bag, Servers, OuterOwner);	
	}
};
...
/**
 Description:
  List out available CLI servers

Usage:
  Beamable.Tools server ps [options]

Options:
  --version <version>                        Only match servers that match the given version
  --owner <owner>                            Only match servers that match the given owner
  --port <port>                              Only match servers that match the given port
  --pid <pid>                                Only match servers that match the given process id
 ...
 */
UCLASS()
class UBeamCliServerPsCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliServerPsStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliServerPsStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
