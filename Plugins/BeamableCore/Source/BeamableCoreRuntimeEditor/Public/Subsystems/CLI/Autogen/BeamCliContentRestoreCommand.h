...
#include "BeamCliContentRestoreCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliContentRestoreStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	TArray<FString> RestoredContents = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<FString>(TEXT("RestoredContents"), RestoredContents, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeArray<FString>(TEXT("RestoredContents"), Bag, RestoredContents, OuterOwner);	
	}
};


UCLASS(BlueprintType)
class UBeamCliContentRestoreErrorContentRestoreErrorReportStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<FString> AvailableSnapshots = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FString Message = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	FString Invocation = {};
	...
	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<FString>(TEXT("AvailableSnapshots"), AvailableSnapshots, Serializer);
	...
	virtual void BeamDeserializeProperties(const TSharedPtr<FJsonObject>& Bag) override
	{
		UBeamJsonUtils::DeserializeArray<FString>(TEXT("AvailableSnapshots"), Bag, AvailableSnapshots, OuterOwner);
...
/**
 Description:
  Restore content data based on a local snapshot, remote manifest ID or manifest file

Usage:
  Beamable.Tools content restore [options]

Options:
  --manifest-id <manifest-id>                Defines the name of the manifest on which the snapshot will be restored. The default value is `global` [default: global]
  ...
  --refresh-token <refresh-token>            A Refresh Token to use for the requests. It overwrites the logged in user stored in auth.beam.json for THIS INVOCATION ONLY
 ...
 */
UCLASS()
class UBeamCliContentRestoreCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliContentRestoreStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliContentRestoreStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;

	inline static FString StreamTypeErrorContentRestoreErrorReport = FString(TEXT("errorContentRestoreErrorReport"));
	UPROPERTY() TArray<UBeamCliContentRestoreErrorContentRestoreErrorReportStreamData*> ErrorContentRestoreErrorReportStream;
	UPROPERTY() TArray<int64> ErrorContentRestoreErrorReportTimestamps;
	TFunction<void (TArray<UBeamCliContentRestoreErrorContentRestoreErrorReportStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnErrorContentRestoreErrorReportStreamOutput;	
