...
#include "Subsystems/CLI/Autogen/StreamData/FetchCommandLogRecordStreamData.h"
#include "BeamCliTelemetryLogsCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliTelemetryLogsStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	TArray<UFetchCommandLogRecordStreamData*> AllLogsFound = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<UFetchCommandLogRecordStreamData*>(TEXT("allLogsFound"), AllLogsFound, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeArray<UFetchCommandLogRecordStreamData*>(TEXT("allLogsFound"), Bag, AllLogsFound, OuterOwner);	
	}
};
...
Usage:
  Beamable.Tools telemetry logs [<filter>] [options]

Arguments:
  <filter>  Value to be matched with the log message body []

Options:
  --id <id>                                  Filter logs by doing a full match with the service name
  --log-level <log-level>                    Filter logs by doing a full match with the Log Level. Available values are: ["Trace", "Debug", "Information", "Warning", "Error", "Critical", "None"]
  --limit <limit>                            Sets a max number of rows to be retrieved by this command [default: 100]
  --full-match                               If set, this will make the body message match be a full exact match
  --from <from>                              The amount of time to go back and retrieve logs. Examples: 12d (12 days), 5m (5 minutes), 48h (48 hours)
  --asc, --ascending                         If set, this will force the order to be ascending instead of the default descending order [default: False]
 ...
 */
UCLASS()
class UBeamCliTelemetryLogsCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliTelemetryLogsStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliTelemetryLogsStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
