...
public:	
	...
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("connectionString"), Bag, ConnectionString);	
	}
};
...
Usage:
  Beamable.Tools services get-connection-string <storage-name> [options]

Arguments:
  <storage-name>  The name of the Microstorage

Options:
  -r, --remote                               The Microstorage remote connection string
  -q, --quiet                                Ignores confirmation step
 ...
 */
UCLASS()
class UBeamCliServicesGetConnectionStringCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliServicesGetConnectionStringStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliServicesGetConnectionStringStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
