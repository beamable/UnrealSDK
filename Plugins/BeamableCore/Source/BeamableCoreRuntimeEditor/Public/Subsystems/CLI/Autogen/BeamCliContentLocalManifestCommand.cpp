...
TSharedPtr<FMonitoredProcess> UBeamCliContentLocalManifestCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("content local-manifest");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliContentLocalManifest Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliContentLocalManifest Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if (Bag.FromJson(MessageJson))
			{
				...
				if(ReceivedStreamType.Equals(StreamType))
				{
					...
					UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliContentLocalManifest Command - Message Received: %s"), *MessageJson);
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliContentLocalManifest Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
