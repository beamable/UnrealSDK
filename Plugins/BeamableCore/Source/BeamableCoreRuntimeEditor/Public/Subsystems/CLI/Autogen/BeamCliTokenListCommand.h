...
#include "Subsystems/CLI/Autogen/StreamData/GetTokenListElementStreamData.h"
...
public:	
	...
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("itemCount"), Bag, ItemCount);
		UBeamJsonUtils::DeserializeArray<UGetTokenListElementStreamData*>(TEXT("items"), Bag, Items, OuterOwner);	
	}
};
...
/**
 Description:
  List tokens for a player or account

Usage:
  Beamable.Tools token list [options]

Options:
  -i, --account-id, --id, --player-id <id>   The player or account id to fetch tokens for. When blank, the current player id will be used
  -o, --offset <offset>                      The offset
  -l, --length <length>                      The max size of the response
  -c, --cid <cid>                            A cid
  -p, --pid <pid>                            A pid
 ...
 */
UCLASS()
class UBeamCliTokenListCommand : public UBeamCliCommand
{
	GENERATED_BODY()
