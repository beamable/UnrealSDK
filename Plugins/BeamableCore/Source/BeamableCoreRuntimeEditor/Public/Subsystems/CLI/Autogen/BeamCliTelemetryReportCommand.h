...
#include "Subsystems/CLI/Autogen/StreamData/TelemetryReportStatusStreamData.h"
#include "BeamCliTelemetryReportCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliTelemetryReportStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	TArray<UTelemetryReportStatusStreamData*> AllStatus = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<UTelemetryReportStatusStreamData*>(TEXT("AllStatus"), AllStatus, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeArray<UTelemetryReportStatusStreamData*>(TEXT("AllStatus"), Bag, AllStatus, OuterOwner);	
	}
};
...
/**
 Description:
  [INTERNAL] Sends custom telemetry data to CLI to be saved in file. Runs `beam otel push` to send this data to ClickHouse

Usage:
  Beamable.Tools telemetry report [options]

Options:
  --paths <paths>                            All paths to files that contain custom log data to be later exported to clickhouse
 ...
 */
UCLASS()
class UBeamCliTelemetryReportCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliTelemetryReportStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliTelemetryReportStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
