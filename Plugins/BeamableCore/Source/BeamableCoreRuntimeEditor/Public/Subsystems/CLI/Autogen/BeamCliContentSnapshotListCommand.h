...
#include "Subsystems/CLI/Autogen/StreamData/ManifestSnapshotItemStreamData.h"
...
#include "BeamCliContentSnapshotListCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliContentSnapshotListStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	TArray<UManifestSnapshotItemStreamData*> SharedSnapshots = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<UManifestSnapshotItemStreamData*> LocalSnapshots = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeArray<UManifestSnapshotItemStreamData*>(TEXT("SharedSnapshots"), SharedSnapshots, Serializer);
		UBeamJsonUtils::SerializeArray<UManifestSnapshotItemStreamData*>(TEXT("LocalSnapshots"), LocalSnapshots, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeArray<UManifestSnapshotItemStreamData*>(TEXT("SharedSnapshots"), Bag, SharedSnapshots, OuterOwner);
		UBeamJsonUtils::DeserializeArray<UManifestSnapshotItemStreamData*>(TEXT("LocalSnapshots"), Bag, LocalSnapshots, OuterOwner);	
	}
};
...
/**
 Description:
  Find and list all shared (.beamable/content-snapshots/[PID]) and local (.beamable/temp/content-snapshots/[PID]) snapshots

Usage:
  Beamable.Tools content snapshot-list [options]

Options:
  --manifest-id <manifest-id>                Defines the name of the manifest that will be used to compare the changes between the manifest and the snapshot. The default value is `global` [default: global]
  --pid <pid>                                An optional field to set the PID from where you would like to get the snapshot to list. The default will get for all the realms []
 ...
 */
UCLASS()
class UBeamCliContentSnapshotListCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliContentSnapshotListStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliContentSnapshotListStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
