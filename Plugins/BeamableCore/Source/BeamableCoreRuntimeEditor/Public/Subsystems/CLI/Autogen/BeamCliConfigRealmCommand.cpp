#include "BeamCliConfigRealmCommand.h"

#include "BeamLogging.h"
#include "Misc/MonitoredProcess.h"
#include "Serialization/JsonSerializerMacros.h"
		
TSharedPtr<FMonitoredProcess> UBeamCliConfigRealmCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("config realm");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliConfigRealm Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliConfigRealm Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if (Bag.FromJson(MessageJson))
			{
				...
				{
					UBeamCliConfigRealmStreamData* Data = NewObject<UBeamCliConfigRealmStreamData>(this);
					...
					UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliConfigRealm Command - Message Received: %s"), *MessageJson);
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliConfigRealm Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
