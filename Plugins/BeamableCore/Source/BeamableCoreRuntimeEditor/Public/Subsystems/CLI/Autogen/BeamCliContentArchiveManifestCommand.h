...
#include "BeamCliContentArchiveManifestCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliContentArchiveManifestStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools content archive-manifest <manifest-id> [options]

Arguments:
  <manifest-id>  The manifest id to archive

Options:
  --unarchive                                When true, restore an archived manifest
 ...
 */
UCLASS()
class UBeamCliContentArchiveManifestCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliContentArchiveManifestStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliContentArchiveManifestStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
