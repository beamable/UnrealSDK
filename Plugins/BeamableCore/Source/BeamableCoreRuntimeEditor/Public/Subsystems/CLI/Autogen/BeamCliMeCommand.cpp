...
TSharedPtr<FMonitoredProcess> UBeamCliMeCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("me --reporter-use-fatal");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliMe Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliMe Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if(StreamType.Equals(FBeamCliMeStreamData::StreamTypeName))
			{
				FBeamCliMeStreamData Data;
				FJsonObjectConverter::JsonObjectToUStruct(DataJson, FBeamCliMeStreamData::StaticStruct(), &Data);

				Stream.Add(Data);
				Timestamps.Add(Timestamp);

				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliMe Command - Message Received: %s"), *MessageJson);
