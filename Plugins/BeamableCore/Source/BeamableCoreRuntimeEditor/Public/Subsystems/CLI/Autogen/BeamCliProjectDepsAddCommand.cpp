...
TSharedPtr<FMonitoredProcess> UBeamCliProjectDepsAddCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("project deps add");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsAdd Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsAdd Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsAdd Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
