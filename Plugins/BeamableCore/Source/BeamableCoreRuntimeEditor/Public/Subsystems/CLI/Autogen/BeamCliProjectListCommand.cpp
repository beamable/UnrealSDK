#include "BeamCliProjectListCommand.h"

#include "BeamLogging.h"
#include "Misc/MonitoredProcess.h"
#include "Serialization/JsonSerializerMacros.h"
		
TSharedPtr<FMonitoredProcess> UBeamCliProjectListCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("project list");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectList Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectList Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if (Bag.FromJson(MessageJson))
			{
				...
				{
					UBeamCliProjectListStreamData* Data = NewObject<UBeamCliProjectListStreamData>(this);
					...
					UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectList Command - Message Received: %s"), *MessageJson);
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectList Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
