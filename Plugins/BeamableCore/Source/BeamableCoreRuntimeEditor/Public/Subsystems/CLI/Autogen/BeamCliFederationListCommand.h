...
class UBeamCliFederationListStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	virtual void BeamDeserializeProperties(const TSharedPtr<FJsonObject>& Bag) override
	{
		...
		UBeamJsonUtils::DeserializeArray<UServiceFederationsStreamData*>(TEXT("services"), Bag, Services, OuterOwner);	
	}
};
 ...
 */
UCLASS()
class UBeamCliFederationListCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliFederationListStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliFederationListStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
