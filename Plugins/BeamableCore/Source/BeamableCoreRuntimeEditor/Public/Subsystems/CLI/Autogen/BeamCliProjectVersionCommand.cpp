#include "BeamCliProjectVersionCommand.h"

#include "BeamLogging.h"
#include "Misc/MonitoredProcess.h"
#include "Serialization/JsonSerializerMacros.h"
		
TSharedPtr<FMonitoredProcess> UBeamCliProjectVersionCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("project version");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectVersion Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectVersion Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if (Bag.FromJson(MessageJson))
			{
				...
				if(ReceivedStreamType.Equals(StreamType))
				{
					...
					UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectVersion Command - Message Received: %s"), *MessageJson);
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectVersion Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
