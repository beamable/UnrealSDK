...
TSharedPtr<FMonitoredProcess> UBeamCliProjectOpenSwaggerCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	...
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectOpenSwagger Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectOpenSwagger Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectOpenSwagger Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
