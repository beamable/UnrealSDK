...
#include "BeamCliTelemetryCollectorPsCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliTelemetryCollectorPsStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
UCLASS(BlueprintType)
class UBeamCliTelemetryCollectorPsExtraStreamStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	int64 FolderSize = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	int64 LastPublishTimestamp = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("FileCount"), FileCount, Serializer);
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("FolderSize"), FolderSize, Serializer);
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("LastPublishTimestamp"), LastPublishTimestamp, Serializer);	
	}
	...
	virtual void BeamDeserializeProperties(const TSharedPtr<FJsonObject>& Bag) override
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("FileCount"), Bag, FileCount);
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("FolderSize"), Bag, FolderSize);
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("LastPublishTimestamp"), Bag, LastPublishTimestamp);	
	}
};
...
/**
 Description:
  Starts a stream of messages containing the status of the collector

Usage:
  Beamable.Tools telemetry collector ps [options]
 ...
 */
UCLASS()
class UBeamCliTelemetryCollectorPsCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliTelemetryCollectorPsStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliTelemetryCollectorPsStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;

	inline static FString StreamTypeExtraStream = FString(TEXT("extraStream"));
	UPROPERTY() TArray<UBeamCliTelemetryCollectorPsExtraStreamStreamData*> ExtraStreamStream;
	UPROPERTY() TArray<int64> ExtraStreamTimestamps;
	TFunction<void (TArray<UBeamCliTelemetryCollectorPsExtraStreamStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnExtraStreamStreamOutput;	
