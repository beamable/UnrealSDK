...
#include "BeamCliContentSnapshotCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliContentSnapshotStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	FString SnapshotFullPath = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("SnapshotFullPath"), SnapshotFullPath, Serializer);	
	}
	...
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("SnapshotFullPath"), Bag, SnapshotFullPath);	
	}
};
...
/**
 Description:
  Save a manifest as a contents snapshot

Usage:
  Beamable.Tools content snapshot [options]

Options:
  --manifest-id <manifest-id>                Defines the name of the manifest that the snapshot will be created from. The default value is `global` [default: global]
  -n, --name <name>                          Defines the name for the snapshot to be created []
  -t, --snapshot-type <Local|Shared>         Defines where the snapshot will be stored to.
                                             Local => Will save the snapshot under `.beamable/temp/content-snapshots/[PID]` folder
                                             Shared => Will save the snapshot under `.beamable/content-snapshots/[PID]` folder [default: Local]
 ...
 */
UCLASS()
class UBeamCliContentSnapshotCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliContentSnapshotStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliContentSnapshotStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
