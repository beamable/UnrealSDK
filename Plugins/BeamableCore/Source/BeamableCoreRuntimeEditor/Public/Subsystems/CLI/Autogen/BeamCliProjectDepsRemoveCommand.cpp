...
TSharedPtr<FMonitoredProcess> UBeamCliProjectDepsRemoveCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("project deps remove");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsRemove Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsRemove Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			{
				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProjectDepsRemove Command - Skipping non-JSON message: %s"), *MessageJson);
			}			
		}
	});
