...
Options:
  ...
  --stop-reason <stop-reason>                      A message to send to the running service when it is terminated
  -mpc, --max-parallel-count <max-parallel-count>  Maximum number of parallel services builds [default: 0]
...
public:
	...
	TFunction<void (TArray<UBeamCliProjectBuildStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
