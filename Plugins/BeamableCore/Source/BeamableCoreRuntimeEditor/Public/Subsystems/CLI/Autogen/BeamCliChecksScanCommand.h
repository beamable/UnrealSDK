...
#include "Subsystems/CLI/Autogen/StreamData/RequiredFileEditStreamData.h"
#include "BeamCliChecksScanCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliChecksScanStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	FString BeamoId = {};
	UPROPERTY(EditAnywhere, BlueprintReadWrite)
	TArray<URequiredFileEditStreamData*> FileEdits = {};

	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("beamoId"), BeamoId, Serializer);
		UBeamJsonUtils::SerializeArray<URequiredFileEditStreamData*>(TEXT("fileEdits"), FileEdits, Serializer);	
	}
	...
	virtual void BeamDeserializeProperties(const TSharedPtr<FJsonObject>& Bag) override
	{
		...
		UBeamJsonUtils::DeserializeArray<URequiredFileEditStreamData*>(TEXT("fileEdits"), Bag, FileEdits, OuterOwner);	
	}
};


UCLASS(BlueprintType)
class UBeamCliChecksScanFixedKeyStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools checks scan [options]

Options:
  -f, --fix <fix>                            Automatically fix known issues for the given code prefixes, or * to fix everything
 ...
 */
UCLASS()
class UBeamCliChecksScanCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliChecksScanStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliChecksScanStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;

	inline static FString StreamTypeFixedKey = FString(TEXT("fixed"));
	UPROPERTY() TArray<UBeamCliChecksScanFixedKeyStreamData*> FixedKeyStream;
	UPROPERTY() TArray<int64> FixedKeyTimestamps;
	TFunction<void (TArray<UBeamCliChecksScanFixedKeyStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnFixedKeyStreamOutput;	
