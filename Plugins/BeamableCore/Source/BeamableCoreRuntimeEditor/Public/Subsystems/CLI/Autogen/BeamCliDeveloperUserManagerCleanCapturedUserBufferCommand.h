...
#include "BeamCliDeveloperUserManagerCleanCapturedUserBufferCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliDeveloperUserManagerCleanCapturedUserBufferStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
/**
 Description:
  Clean up the captured users files based on a rolling buffer, the criteria is the oldest to the newer

Usage:
  Beamable.Tools developer-user-manager clean-captured-user-buffer [options]
 ...
 */
UCLASS()
class UBeamCliDeveloperUserManagerCleanCapturedUserBufferCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliDeveloperUserManagerCleanCapturedUserBufferStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliDeveloperUserManagerCleanCapturedUserBufferStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
