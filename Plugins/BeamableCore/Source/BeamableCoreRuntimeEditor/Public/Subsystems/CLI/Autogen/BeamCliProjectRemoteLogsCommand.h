...
#include "BeamCliProjectRemoteLogsCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliProjectRemoteLogsStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	{
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("raw"), Bag, Raw);
...
Usage:
  Beamable.Tools project remote-logs <service-id> [options]

Arguments:
  <service-id>  The beamo id for the service to get logs for

Options:
  -f, --filter <filter>                       A text filter for log searching
  -sl, --server-log-level <server-log-level>  A log level filter for searching
  --from <from>                               A timestamp filter, where logs must be newer than this time. Must be an exact date time string, or a relative time string. Relative time strings are in the format <number><unit>. The unit is either s (seconds), m (minutes), h (hours), or d (days). To represent 5 minutes in the past, use the term '5m'
 ...
 */
UCLASS()
class UBeamCliProjectRemoteLogsCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliProjectRemoteLogsStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliProjectRemoteLogsStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
