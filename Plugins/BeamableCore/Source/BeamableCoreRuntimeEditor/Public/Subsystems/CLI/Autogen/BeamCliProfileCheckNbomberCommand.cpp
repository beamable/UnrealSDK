...
TSharedPtr<FMonitoredProcess> UBeamCliProfileCheckNbomberCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	FString Params = ("profile check-nbomber --reporter-use-fatal");
	for (const auto& CommandParam : CommandParams)
		Params.Appendf(TEXT(" %s"), *CommandParam);
	Params = PrepareParams(Params);
	UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProfileCheckNbomber Command - Invocation: %s %s"), *PathToCli, *Params)
	...
	{
		UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProfileCheckNbomber Command - Std Out: %s"), *Out);
		FString OutCopy = Out;
		FString MessageJson;
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			{
				UBeamCliProfileCheckNbomberStreamData* Data = NewObject<UBeamCliProfileCheckNbomberStreamData>();
				Data->BeamDeserializeProperties(DataJson);

				Stream.Add(Data);
				Timestamps.Add(Timestamp);

				UE_LOG(LogBeamCli, Verbose, TEXT("BeamCliProfileCheckNbomber Command - Message Received: %s"), *MessageJson);
