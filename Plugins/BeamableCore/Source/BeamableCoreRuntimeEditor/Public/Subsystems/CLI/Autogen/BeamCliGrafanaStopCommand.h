...
#include "BeamCliGrafanaStopCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliGrafanaStopStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
public:	
	...
	virtual void BeamSerializeProperties(TUnrealJsonSerializer& Serializer) const override
	{
		...
		UBeamJsonUtils::SerializeRawPrimitive(TEXT("containerName"), ContainerName, Serializer);	
	}
	...
	virtual void BeamDeserializeProperties(const TSharedPtr<FJsonObject>& Bag) override
	{
		...
		UBeamJsonUtils::DeserializeRawPrimitive(TEXT("containerName"), Bag, ContainerName);	
	}
};
...
Usage:
  Beamable.Tools grafana stop [options]
 ...
 */
UCLASS()
class UBeamCliGrafanaStopCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliGrafanaStopStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliGrafanaStopStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
