...
#include "BeamCliTelemetrySetConfigCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliTelemetrySetConfigStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools telemetry set-config <cli-log-level> <cli-telemetry-max-size> [<cli-allow-telemetry>] [options]

Arguments:
  <cli-log-level>           The minimum Open Telemetry LogLevel to be sent to Clickhouse, this needs to be a valid LogLevel converted to string value
  <cli-telemetry-max-size>  The maximum size in bytes for saved Otel data
  <cli-allow-telemetry>     Define if the telemetry is allowed to push informations to beamable server
 ...
 */
UCLASS()
class UBeamCliTelemetrySetConfigCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliTelemetrySetConfigStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (TArray<UBeamCliTelemetrySetConfigStreamData*>& StreamData, TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
