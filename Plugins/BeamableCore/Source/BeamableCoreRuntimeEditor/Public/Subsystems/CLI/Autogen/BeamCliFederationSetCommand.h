...
#include "BeamCliFederationSetCommand.generated.h"


UCLASS(BlueprintType)
class UBeamCliFederationSetStreamData : public UObject, public IBeamJsonSerializableUObject
{
	GENERATED_BODY()
...
Usage:
  Beamable.Tools federation set <microservice> [options]

Arguments:
  <microservice>  The BeamoId of the microservice to add

Options:
  -c, --clear                            Erase all federations
  --fed-id <fed-id>                      A federation id, must be in a parallel layout to the --fed-type option
  --fed-type <fed-type>                  A federation type, must be in a parallel layout to the --fed-id option
 ...
 */
UCLASS()
class UBeamCliFederationSetCommand : public UBeamCliCommand
{
	GENERATED_BODY()

public:
	inline static FString StreamType = FString(TEXT("stream"));
	UPROPERTY() TArray<UBeamCliFederationSetStreamData*> Stream;
	UPROPERTY() TArray<int64> Timestamps;
	TFunction<void (const TArray<UBeamCliFederationSetStreamData*>& StreamData, const TArray<int64>& Timestamps, const FBeamOperationHandle& Op)> OnStreamOutput;	
