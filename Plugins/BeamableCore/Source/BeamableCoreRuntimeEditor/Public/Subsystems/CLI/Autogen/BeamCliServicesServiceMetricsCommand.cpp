...
{
	FString Params = ("services service-metrics --reporter-use-fatal");
	...
	CliProcess->OnOutput().BindLambda([this, Op](const FString& Out)
	{
		...
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			{
				UBeamCliServicesServiceMetricsStreamData* Data = NewObject<UBeamCliServicesServiceMetricsStreamData>();
