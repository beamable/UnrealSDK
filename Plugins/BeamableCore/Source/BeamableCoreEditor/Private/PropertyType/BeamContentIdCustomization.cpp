...
void FBeamContentIdCustomization::CustomizeHeader(TSharedRef<IPropertyHandle> StructPropertyHandle, FDetailWidgetRow& HeaderRow, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
	...
	if (const auto StructProp = CastField<FStructProperty>(StructPropertyHandle->GetProperty()))
	{
		if (StructProp->Struct == FBeamContentId::StaticStruct())
			AsStringPropertyHandle = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FBeamContentId, AsString));

		if (StructProp->Struct == FBeamContentLink::StaticStruct())
			AsStringPropertyHandle = StructPropertyHandle->GetChildHandle(GET_MEMBER_NAME_CHECKED(FBeamContentLink, AsString));
	}

	check(AsStringPropertyHandle.IsValid());


	const auto TypeComboBoxOpenHandler = FOnComboBoxOpening::CreateRaw(this, &FBeamContentIdCustomization::OnTypeComboBoxOpening, AsStringPropertyHandle);
	const auto TypeComboBoxSelectHandler = SNameComboBox::FOnNameSelectionChanged::CreateRaw(this, &FBeamContentIdCustomization::OnTypeSelected, AsStringPropertyHandle);

	const auto IdsComboBoxOpenHandler = FOnComboBoxOpening::CreateRaw(this, &FBeamContentIdCustomization::OnIdComboBoxOpening, AsStringPropertyHandle);
	const auto IdsComboBoxSelectHandler = SNameComboBox::FOnNameSelectionChanged::CreateRaw(this, &FBeamContentIdCustomization::OnIdSelected, AsStringPropertyHandle);
			...
			.ValueContent()[
				...
				[
					SAssignNew(TypeComboBox, SNameComboBox) // you can display any widget here
						...
						.InitiallySelectedItem(SelectedTypeName) // the default or previous selected value
						.OnComboBoxOpening(TypeComboBoxOpenHandler) // this event is defined by the SNameComboBox
				...
				[
					SAssignNew(IdComboBox, SNameComboBox) // you can display any widget here
						...
						.InitiallySelectedItem(SelectedIdName) // the default or previous selected value
						.OnComboBoxOpening(IdsComboBoxOpenHandler) // this event is defined by the SNameComboBox
...
void FBeamContentIdCustomization::CustomizeChildren(TSharedRef<IPropertyHandle> StructPropertyHandle, IDetailChildrenBuilder& StructBuilder, IPropertyTypeCustomizationUtils& StructCustomizationUtils)
{
...
void FBeamContentIdCustomization::OnTypeSelected(TSharedPtr<FName> ItemSelected, ESelectInfo::Type SelectInfo, const TSharedPtr<IPropertyHandle> AsStringPropertyHandle)
{
	if (AsStringPropertyHandle->IsValidHandle())
	{
		FString Id;
...
void FBeamContentIdCustomization::OnIdSelected(TSharedPtr<FName> ItemSelected, ESelectInfo::Type SelectInfo, const TSharedPtr<IPropertyHandle> AsStringPropertyHandle)
{
...
TSharedPtr<FName> FBeamContentIdCustomization::GetSelectedId(const TSharedPtr<IPropertyHandle> AsStringPropertyHandle) const
{
...
void FBeamContentIdCustomization::GetContentOptions(TArray<TSharedPtr<FName>>& TypeNames, TMap<FName, TArray<TSharedPtr<FName>>>& PerTypeNameIds)
{
