...
inline TSharedPtr<FMonitoredProcess> UBeamCliServicesPsCommand::RunImpl(const TArray<FString>& CommandParams, const FBeamOperationHandle& Op)
{
	...
	CliProcess->OnOutput().BindLambda([this, Op](const FString& Out)
	{
		...
		while (ConsumeMessageFromOutput(OutCopy, MessageJson))
		{
			...
			if(StreamType.Equals(FBeamCliServicesPsStreamData::StreamTypeName))
			{
				FBeamCliServicesPsStreamData Data;
				FJsonObjectConverter::JsonObjectToUStruct(DataJson, FBeamCliServicesPsStreamData::StaticStruct(), &Data);
