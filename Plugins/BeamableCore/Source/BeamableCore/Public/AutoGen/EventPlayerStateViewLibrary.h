...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventPlayerStateView To JSON String")
	static FString EventPlayerStateViewToJsonString(const UEventPlayerStateView* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventPlayerStateView", meta=(DefaultToSelf="Outer", AdvancedDisplay="CurrentPhase, GroupRewards, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventPlayerStateView", meta=(NativeBreakFunc))
