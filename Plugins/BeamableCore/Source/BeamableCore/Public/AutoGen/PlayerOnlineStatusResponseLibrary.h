...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="PlayerOnlineStatusResponse To JSON String")
	static FString PlayerOnlineStatusResponseToJsonString(const UPlayerOnlineStatusResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make PlayerOnlineStatusResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UPlayerOnlineStatusResponse* Make(bool bOnline, int64 PlayerId, int64 LastSeen, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break PlayerOnlineStatusResponse", meta=(NativeBreakFunc))
