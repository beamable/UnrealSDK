...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Friends|4 - Json", DisplayName="PlayerIdRequestBody To JSON String")
	static FString PlayerIdRequestBodyToJsonString(const UPlayerIdRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Friends|3 - Backend", DisplayName="Make PlayerIdRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UPlayerIdRequestBody* Make(FBeamGamerTag PlayerId, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Friends|3 - Backend", DisplayName="Break PlayerIdRequestBody", meta=(NativeBreakFunc))
