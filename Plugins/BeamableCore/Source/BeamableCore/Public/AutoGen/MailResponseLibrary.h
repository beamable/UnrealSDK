...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Mail|4 - Json", DisplayName="MailResponse To JSON String")
	static FString MailResponseToJsonString(const UMailResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Make MailResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Result, Outer", NativeMakeFunc))
	static UMailResponse* Make(FOptionalMessage Result, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Break MailResponse", meta=(NativeBreakFunc))
