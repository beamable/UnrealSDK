...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="TournamentCurrencyReward To JSON String")
	static FString TournamentCurrencyRewardToJsonString(const UTournamentCurrencyReward* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make TournamentCurrencyReward", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UTournamentCurrencyReward* Make(FString Symbol, int64 Amount, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break TournamentCurrencyReward", meta=(NativeBreakFunc))
