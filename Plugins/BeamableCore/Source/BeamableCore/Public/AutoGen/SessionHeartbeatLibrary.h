...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="SessionHeartbeat To JSON String")
	static FString SessionHeartbeatToJsonString(const USessionHeartbeat* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make SessionHeartbeat", meta=(DefaultToSelf="Outer", AdvancedDisplay="Heartbeat, Outer", NativeMakeFunc))
	static USessionHeartbeat* Make(int64 Gt, FOptionalInt64 Heartbeat, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break SessionHeartbeat", meta=(NativeBreakFunc))
