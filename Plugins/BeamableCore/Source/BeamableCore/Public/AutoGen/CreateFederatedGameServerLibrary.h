...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CreateFederatedGameServer To JSON String")
	static FString CreateFederatedGameServerToJsonString(const UCreateFederatedGameServer* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CreateFederatedGameServer", meta=(DefaultToSelf="Outer", AdvancedDisplay="MatchType, Outer", NativeMakeFunc))
	static UCreateFederatedGameServer* Make(FOptionalBeamContentId MatchType, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CreateFederatedGameServer", meta=(NativeBreakFunc))
