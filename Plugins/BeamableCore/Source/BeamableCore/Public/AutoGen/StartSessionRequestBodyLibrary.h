...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="StartSessionRequestBody To JSON String")
	static FString StartSessionRequestBodyToJsonString(const UStartSessionRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make StartSessionRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Source, Shard, Locale, Language, Time, Platform, Gamer, Device, CustomParams, DeviceParams, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break StartSessionRequestBody", meta=(NativeBreakFunc))
