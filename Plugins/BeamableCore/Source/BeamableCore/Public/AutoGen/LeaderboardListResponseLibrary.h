...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="LeaderboardListResponse To JSON String")
	static FString LeaderboardListResponseToJsonString(const ULeaderboardListResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make LeaderboardListResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static ULeaderboardListResponse* Make(int32 Total, int32 Offset, TArray<FString> NameList, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break LeaderboardListResponse", meta=(NativeBreakFunc))
