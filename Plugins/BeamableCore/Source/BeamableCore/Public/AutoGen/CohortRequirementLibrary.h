...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CohortRequirement To JSON String")
	static FString CohortRequirementToJsonString(const UCohortRequirement* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CohortRequirement", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCohortRequirement* Make(FString Trial, FString Cohort, FString Constraint, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CohortRequirement", meta=(NativeBreakFunc))
