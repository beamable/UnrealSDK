...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Content|4 - Json", DisplayName="SaveContentRequestBody To JSON String")
	static FString SaveContentRequestBodyToJsonString(const USaveContentRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Make SaveContentRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static USaveContentRequestBody* Make(TArray<UContentDefinition*> Content, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Break SaveContentRequestBody", meta=(NativeBreakFunc))
