...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CurrencyProperty To JSON String")
	static FString CurrencyPropertyToJsonString(const UCurrencyProperty* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CurrencyProperty", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCurrencyProperty* Make(FString Name, FString Value, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CurrencyProperty", meta=(NativeBreakFunc))
