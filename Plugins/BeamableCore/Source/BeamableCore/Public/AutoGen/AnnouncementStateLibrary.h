...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="AnnouncementState To JSON String")
	static FString AnnouncementStateToJsonString(const UAnnouncementState* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make AnnouncementState", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UAnnouncementState* Make(bool bIsClaimed, bool bIsDeleted, bool bIsRead, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break AnnouncementState", meta=(NativeBreakFunc))
