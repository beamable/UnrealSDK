...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventPhaseRuntime To JSON String")
	static FString EventPhaseRuntimeToJsonString(const UEventPhaseRuntime* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventPhaseRuntime", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEventPhaseRuntime* Make(FString Name, int64 StartTime, int64 EndTime, TArray<UEventRule*> Rules, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventPhaseRuntime", meta=(NativeBreakFunc))
