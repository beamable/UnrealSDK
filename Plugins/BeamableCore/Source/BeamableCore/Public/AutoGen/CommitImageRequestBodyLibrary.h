...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CommitImageRequestBody To JSON String")
	static FString CommitImageRequestBodyToJsonString(const UCommitImageRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CommitImageRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCommitImageRequestBody* Make(UBeamoBasicReference* Service, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CommitImageRequestBody", meta=(NativeBreakFunc))
