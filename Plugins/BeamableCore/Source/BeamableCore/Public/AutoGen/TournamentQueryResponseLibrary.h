...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="TournamentQueryResponse To JSON String")
	static FString TournamentQueryResponseToJsonString(const UTournamentQueryResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make TournamentQueryResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UTournamentQueryResponse* Make(TArray<UTournamentClientView*> Tournaments, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break TournamentQueryResponse", meta=(NativeBreakFunc))
