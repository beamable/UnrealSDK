...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="Period To JSON String")
	static FString PeriodToJsonString(const UPeriod* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make Period", meta=(DefaultToSelf="Outer", AdvancedDisplay="End, Outer", NativeMakeFunc))
	static UPeriod* Make(FString Start, FOptionalString End, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break Period", meta=(NativeBreakFunc))
