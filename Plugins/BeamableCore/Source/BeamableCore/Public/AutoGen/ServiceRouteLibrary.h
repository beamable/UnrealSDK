...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ServiceRoute To JSON String")
	static FString ServiceRouteToJsonString(const UServiceRoute* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ServiceRoute", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UServiceRoute* Make(FString Service, FString Endpoint, EWebhookServiceType ServiceTypeStr, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ServiceRoute", meta=(NativeBreakFunc))
