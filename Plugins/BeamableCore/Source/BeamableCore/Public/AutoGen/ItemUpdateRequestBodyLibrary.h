...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Inventory|4 - Json", DisplayName="ItemUpdateRequestBody To JSON String")
	static FString ItemUpdateRequestBodyToJsonString(const UItemUpdateRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Inventory|3 - Backend", DisplayName="Make ItemUpdateRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UItemUpdateRequestBody* Make(FString ContentId, int64 Id, TArray<UItemProperty*> Properties, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Inventory|3 - Backend", DisplayName="Break ItemUpdateRequestBody", meta=(NativeBreakFunc))
