...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Mail|4 - Json", DisplayName="MyMailUpdate To JSON String")
	static FString MyMailUpdateToJsonString(const UMyMailUpdate* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Make MyMailUpdate", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UMyMailUpdate* Make(int64 Id, UUpdateMailRequestBody* Update, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Break MyMailUpdate", meta=(NativeBreakFunc))
