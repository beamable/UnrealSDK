...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Realms|4 - Json", DisplayName="InFlightFailureResponse To JSON String")
	static FString InFlightFailureResponseToJsonString(const UInFlightFailureResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Make InFlightFailureResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UInFlightFailureResponse* Make(TArray<UInFlightFailure*> Failures, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Break InFlightFailureResponse", meta=(NativeBreakFunc))
