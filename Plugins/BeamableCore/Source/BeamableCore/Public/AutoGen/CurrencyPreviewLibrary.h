...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CurrencyPreview To JSON String")
	static FString CurrencyPreviewToJsonString(const UCurrencyPreview* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CurrencyPreview", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCurrencyPreview* Make(FString Id, int64 Amount, int64 OriginalAmount, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CurrencyPreview", meta=(NativeBreakFunc))
