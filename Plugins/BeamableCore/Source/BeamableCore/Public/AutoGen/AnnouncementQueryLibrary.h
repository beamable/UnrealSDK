...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="AnnouncementQuery To JSON String")
	static FString AnnouncementQueryToJsonString(const UAnnouncementQuery* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make AnnouncementQuery", meta=(DefaultToSelf="Outer", AdvancedDisplay="bIncludeDeleted, Outer", NativeMakeFunc))
	static UAnnouncementQuery* Make(FOptionalBool bIncludeDeleted, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break AnnouncementQuery", meta=(NativeBreakFunc))
