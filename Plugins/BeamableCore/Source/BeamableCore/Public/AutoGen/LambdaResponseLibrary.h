...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Beamo|4 - Json", DisplayName="LambdaResponse To JSON String")
	static FString LambdaResponseToJsonString(const ULambdaResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Make LambdaResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Body, Outer", NativeMakeFunc))
	static ULambdaResponse* Make(int32 StatusCode, FOptionalString Body, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Break LambdaResponse", meta=(NativeBreakFunc))
