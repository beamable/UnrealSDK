...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="GetManifestsRequestBody To JSON String")
	static FString GetManifestsRequestBodyToJsonString(const UGetManifestsRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make GetManifestsRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="bArchived, Limit, Offset, Outer", NativeMakeFunc))
	static UGetManifestsRequestBody* Make(FOptionalBool bArchived, FOptionalInt32 Limit, FOptionalInt32 Offset, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break GetManifestsRequestBody", meta=(NativeBreakFunc))
