...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Presence|4 - Json", DisplayName="PlayersStatusResponse To JSON String")
	static FString PlayersStatusResponseToJsonString(const UPlayersStatusResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Presence|3 - Backend", DisplayName="Make PlayersStatusResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="PlayersStatus, Outer", NativeMakeFunc))
	static UPlayersStatusResponse* Make(FOptionalArrayOfOnlineStatus PlayersStatus, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Presence|3 - Backend", DisplayName="Break PlayersStatusResponse", meta=(NativeBreakFunc))
