...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Beamo|4 - Json", DisplayName="PreSignedUrlsResponse To JSON String")
	static FString PreSignedUrlsResponseToJsonString(const UPreSignedUrlsResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Make PreSignedUrlsResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UPreSignedUrlsResponse* Make(TArray<UBeamoBasicURLResponse*> Response, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Break PreSignedUrlsResponse", meta=(NativeBreakFunc))
