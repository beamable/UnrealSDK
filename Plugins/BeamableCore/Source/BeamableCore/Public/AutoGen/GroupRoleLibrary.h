...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|4 - Json", DisplayName="GroupRole To JSON String")
	static FString GroupRoleToJsonString(const UGroupRole* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|3 - Backend", DisplayName="Make GroupRole", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGroupRole* Make(FString Name, TArray<FString> Permissions, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|3 - Backend", DisplayName="Break GroupRole", meta=(NativeBreakFunc))
