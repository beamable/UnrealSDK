...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventRewardState To JSON String")
	static FString EventRewardStateToJsonString(const UEventRewardState* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventRewardState", meta=(DefaultToSelf="Outer", AdvancedDisplay="Max, Currencies, PendingItemRewards, Items, Obtain, PendingCurrencyRewards, PendingEntitlementRewards, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventRewardState", meta=(NativeBreakFunc))
