...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CompletedStatus To JSON String")
	static FString CompletedStatusToJsonString(const UCompletedStatus* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CompletedStatus", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCompletedStatus* Make(int32 Cycle, int32 Tier, int32 Stage, int32 Delta, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CompletedStatus", meta=(NativeBreakFunc))
