...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Content|4 - Json", DisplayName="GetContentRequestBody To JSON String")
	static FString GetContentRequestBodyToJsonString(const UGetContentRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Make GetContentRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGetContentRequestBody* Make(FBeamContentId ContentId, FString Version, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Break GetContentRequestBody", meta=(NativeBreakFunc))
