...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ActiveListingResponse To JSON String")
	static FString ActiveListingResponseToJsonString(const UActiveListingResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ActiveListingResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UActiveListingResponse* Make(FString StoreSymbol, UPlayerListingView* Listing, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ActiveListingResponse", meta=(NativeBreakFunc))
