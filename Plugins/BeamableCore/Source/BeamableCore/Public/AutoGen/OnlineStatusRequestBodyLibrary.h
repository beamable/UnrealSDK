...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Session|4 - Json", DisplayName="OnlineStatusRequestBody To JSON String")
	static FString OnlineStatusRequestBodyToJsonString(const UOnlineStatusRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Session|3 - Backend", DisplayName="Make OnlineStatusRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UOnlineStatusRequestBody* Make(FString PlayerIds, int64 IntervalSecs, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Session|3 - Backend", DisplayName="Break OnlineStatusRequestBody", meta=(NativeBreakFunc))
