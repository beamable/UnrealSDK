...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="RedisShardRequestBody To JSON String")
	static FString RedisShardRequestBodyToJsonString(const URedisShardRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make RedisShardRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static URedisShardRequestBody* Make(int32 ShardId, FString MasterHost, FString SlaveHosts, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break RedisShardRequestBody", meta=(NativeBreakFunc))
