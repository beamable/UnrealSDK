...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Commerce|4 - Json", DisplayName="GiveCouponReq To JSON String")
	static FString GiveCouponReqToJsonString(const UGiveCouponReq* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Make GiveCouponReq", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGiveCouponReq* Make(FString Listing, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Break GiveCouponReq", meta=(NativeBreakFunc))
