...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Beamo|4 - Json", DisplayName="ServiceStorageStatus To JSON String")
	static FString ServiceStorageStatusToJsonString(const UServiceStorageStatus* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Make ServiceStorageStatus", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UServiceStorageStatus* Make(bool bIsCurrent, bool bIsRunning, FString Id, FString StorageType, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|3 - Backend", DisplayName="Break ServiceStorageStatus", meta=(NativeBreakFunc))
