...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="OnlineStatusResponses To JSON String")
	static FString OnlineStatusResponsesToJsonString(const UOnlineStatusResponses* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make OnlineStatusResponses", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UOnlineStatusResponses* Make(TArray<UPlayerOnlineStatusResponse*> Players, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break OnlineStatusResponses", meta=(NativeBreakFunc))
