...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Beamo|Utils|Make/Break", DisplayName="Make GetTemplatesResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Templates, Outer", NativeMakeFunc))
	static UGetTemplatesResponse* Make(FOptionalArrayOfServiceTemplateView Templates, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Beamo|Utils|Make/Break", DisplayName="Break GetTemplatesResponse", meta=(NativeBreakFunc))
	static void Break(const UGetTemplatesResponse* Serializable, FOptionalArrayOfServiceTemplateView& Templates);
};