...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Push|4 - Json", DisplayName="SendNotification To JSON String")
	static FString SendNotificationToJsonString(const USendNotification* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Push|3 - Backend", DisplayName="Make SendNotification", meta=(DefaultToSelf="Outer", AdvancedDisplay="Title, Body, Outer", NativeMakeFunc))
	static USendNotification* Make(FOptionalString Title, FOptionalString Body, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Push|3 - Backend", DisplayName="Break SendNotification", meta=(NativeBreakFunc))
