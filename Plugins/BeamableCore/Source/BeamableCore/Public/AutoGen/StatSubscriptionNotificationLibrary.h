...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="StatSubscriptionNotification To JSON String")
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make StatSubscriptionNotification", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UStatSubscriptionNotification* Make(TMap<FString, FString> StatsBefore, TMap<FString, FString> StatsAfter, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break StatSubscriptionNotification", meta=(NativeBreakFunc))
