...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ListTokenResponseItem To JSON String")
	static FString ListTokenResponseItemToJsonString(const UListTokenResponseItem* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ListTokenResponseItem", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UListTokenResponseItem* Make(FString Token, FString Platform, FString Device, FString Created, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ListTokenResponseItem", meta=(NativeBreakFunc))
