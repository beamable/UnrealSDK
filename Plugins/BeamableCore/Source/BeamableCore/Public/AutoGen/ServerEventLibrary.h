...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ServerEvent To JSON String")
	static FString ServerEventToJsonString(const UServerEvent* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ServerEvent", meta=(DefaultToSelf="Outer", AdvancedDisplay="Payload, Outer", NativeMakeFunc))
	static UServerEvent* Make(FString Event, bool bToAll, FOptionalString Payload, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ServerEvent", meta=(NativeBreakFunc))
