...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Content|4 - Json", DisplayName="PullManifestRequestBody To JSON String")
	static FString PullManifestRequestBodyToJsonString(const UPullManifestRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Make PullManifestRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Id, Outer", NativeMakeFunc))
	static UPullManifestRequestBody* Make(FBeamPid SourceRealmPid, FOptionalBeamContentManifestId Id, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Break PullManifestRequestBody", meta=(NativeBreakFunc))
