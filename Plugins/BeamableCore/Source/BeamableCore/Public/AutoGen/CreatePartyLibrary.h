...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Party|4 - Json", DisplayName="CreateParty To JSON String")
	static FString CreatePartyToJsonString(const UCreateParty* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Party|3 - Backend", DisplayName="Make CreateParty", meta=(DefaultToSelf="Outer", AdvancedDisplay="Restriction, Leader, MaxSize, Outer", NativeMakeFunc))
	static UCreateParty* Make(FOptionalString Restriction, FOptionalBeamGamerTag Leader, FOptionalInt32 MaxSize, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Party|3 - Backend", DisplayName="Break CreateParty", meta=(NativeBreakFunc))
