...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Party|4 - Json", DisplayName="LeaveParty To JSON String")
	static FString LeavePartyToJsonString(const ULeaveParty* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Party|3 - Backend", DisplayName="Make LeaveParty", meta=(DefaultToSelf="Outer", AdvancedDisplay="PlayerId, Outer", NativeMakeFunc))
	static ULeaveParty* Make(FOptionalBeamGamerTag PlayerId, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Party|3 - Backend", DisplayName="Break LeaveParty", meta=(NativeBreakFunc))
