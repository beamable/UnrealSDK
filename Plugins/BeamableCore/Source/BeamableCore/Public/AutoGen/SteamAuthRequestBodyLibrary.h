...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Payments|4 - Json", DisplayName="SteamAuthRequestBody To JSON String")
	static FString SteamAuthRequestBodyToJsonString(const USteamAuthRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Payments|3 - Backend", DisplayName="Make SteamAuthRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static USteamAuthRequestBody* Make(FString Ticket, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Payments|3 - Backend", DisplayName="Break SteamAuthRequestBody", meta=(NativeBreakFunc))
