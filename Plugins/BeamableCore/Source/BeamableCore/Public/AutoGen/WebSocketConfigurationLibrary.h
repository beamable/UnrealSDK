...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Realms|4 - Json", DisplayName="WebSocketConfiguration To JSON String")
	static FString WebSocketConfigurationToJsonString(const UWebSocketConfiguration* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Make WebSocketConfiguration", meta=(DefaultToSelf="Outer", AdvancedDisplay="Uri, Outer", NativeMakeFunc))
	static UWebSocketConfiguration* Make(FString Provider, FOptionalString Uri, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Break WebSocketConfiguration", meta=(NativeBreakFunc))
