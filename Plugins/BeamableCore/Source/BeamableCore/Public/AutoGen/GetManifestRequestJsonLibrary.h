...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="GetManifestRequestJson To JSON String")
	static FString GetManifestRequestJsonToJsonString(const UGetManifestRequestJson* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make GetManifestRequestJson", meta=(DefaultToSelf="Outer", AdvancedDisplay="Id, Uid, Outer", NativeMakeFunc))
	static UGetManifestRequestJson* Make(FOptionalBeamContentManifestId Id, FOptionalString Uid, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break GetManifestRequestJson", meta=(NativeBreakFunc))
