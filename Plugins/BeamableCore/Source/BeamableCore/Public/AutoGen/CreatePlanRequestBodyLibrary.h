...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CreatePlanRequestBody To JSON String")
	static FString CreatePlanRequestBodyToJsonString(const UCreatePlanRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CreatePlanRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="MongoSrvAddress, MessageBusAnalytics, MessageBusCommon, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CreatePlanRequestBody", meta=(NativeBreakFunc))
