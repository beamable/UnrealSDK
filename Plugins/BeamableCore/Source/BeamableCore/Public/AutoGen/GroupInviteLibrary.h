...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Groups|4 - Json", DisplayName="GroupInvite To JSON String")
	static FString GroupInviteToJsonString(const UGroupInvite* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Groups|3 - Backend", DisplayName="Make GroupInvite", meta=(DefaultToSelf="Outer", AdvancedDisplay="SubGroup, bUseNewRewardsSystem, Outer", NativeMakeFunc))
	static UGroupInvite* Make(int64 GamerTag, FOptionalInt64 SubGroup, FOptionalBool bUseNewRewardsSystem, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Groups|3 - Backend", DisplayName="Break GroupInvite", meta=(NativeBreakFunc))
