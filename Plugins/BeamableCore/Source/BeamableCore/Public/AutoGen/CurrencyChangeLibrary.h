...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CurrencyChange To JSON String")
	static FString CurrencyChangeToJsonString(const UCurrencyChange* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CurrencyChange", meta=(DefaultToSelf="Outer", AdvancedDisplay="OriginalAmount, Outer", NativeMakeFunc))
	static UCurrencyChange* Make(FString Symbol, int64 Amount, FOptionalInt64 OriginalAmount, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CurrencyChange", meta=(NativeBreakFunc))
