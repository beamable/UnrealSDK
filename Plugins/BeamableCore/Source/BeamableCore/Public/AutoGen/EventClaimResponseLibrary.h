...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventClaimResponse To JSON String")
	static FString EventClaimResponseToJsonString(const UEventClaimResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventClaimResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEventClaimResponse* Make(UEventPlayerStateView* View, FString GameRspJson, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventClaimResponse", meta=(NativeBreakFunc))
