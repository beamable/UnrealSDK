...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Commerce|4 - Json", DisplayName="GetCatalogReq To JSON String")
	static FString GetCatalogReqToJsonString(const UGetCatalogReq* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Make GetCatalogReq", meta=(DefaultToSelf="Outer", AdvancedDisplay="Version, Outer", NativeMakeFunc))
	static UGetCatalogReq* Make(FOptionalInt64 Version, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Break GetCatalogReq", meta=(NativeBreakFunc))
