...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="Store To JSON String")
	static FString StoreToJsonString(const UStore* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make Store", meta=(DefaultToSelf="Outer", AdvancedDisplay="bShowInactiveListings, ActiveListingLimit, Choose, RefreshTime, Title, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break Store", meta=(NativeBreakFunc))
