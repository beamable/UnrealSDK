...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="UpdateMailRequestBody To JSON String")
	static FString UpdateMailRequestBodyToJsonString(const UUpdateMailRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make UpdateMailRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="bAcceptAttachments, Body, Expires, Subject, State, Category, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break UpdateMailRequestBody", meta=(NativeBreakFunc))
