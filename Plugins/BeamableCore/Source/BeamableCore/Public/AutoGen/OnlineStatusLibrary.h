...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="OnlineStatus To JSON String")
	static FString OnlineStatusToJsonString(const UOnlineStatus* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make OnlineStatus", meta=(DefaultToSelf="Outer", AdvancedDisplay="LastOnline, bOnline, PlayerId, Status, Description, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break OnlineStatus", meta=(NativeBreakFunc))
