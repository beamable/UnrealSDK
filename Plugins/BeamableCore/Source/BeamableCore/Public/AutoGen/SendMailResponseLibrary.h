...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="SendMailResponse To JSON String")
	static FString SendMailResponseToJsonString(const USendMailResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make SendMailResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="MailId, Outer", NativeMakeFunc))
	static USendMailResponse* Make(FOptionalInt64 MailId, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break SendMailResponse", meta=(NativeBreakFunc))
