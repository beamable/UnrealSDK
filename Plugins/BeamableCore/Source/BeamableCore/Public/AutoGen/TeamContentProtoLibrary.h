...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="TeamContentProto To JSON String")
	static FString TeamContentProtoToJsonString(const UTeamContentProto* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make TeamContentProto", meta=(DefaultToSelf="Outer", AdvancedDisplay="Name, MaxPlayers, MinPlayers, Outer", NativeMakeFunc))
	static UTeamContentProto* Make(FOptionalString Name, FOptionalInt32 MaxPlayers, FOptionalInt32 MinPlayers, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break TeamContentProto", meta=(NativeBreakFunc))
