...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ContentLimits To JSON String")
	static FString ContentLimitsToJsonString(const UContentLimits* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ContentLimits", meta=(DefaultToSelf="Outer", AdvancedDisplay="MaxDistinctContentIds, Outer", NativeMakeFunc))
	static UContentLimits* Make(FOptionalInt32 MaxDistinctContentIds, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ContentLimits", meta=(NativeBreakFunc))
