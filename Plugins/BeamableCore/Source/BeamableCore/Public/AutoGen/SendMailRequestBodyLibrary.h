...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Mail|4 - Json", DisplayName="SendMailRequestBody To JSON String")
	static FString SendMailRequestBodyToJsonString(const USendMailRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Make SendMailRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Body, Expires, PlayerRewards, Subject, Rewards, Id, BodyRef, Attachments, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Mail|3 - Backend", DisplayName="Break SendMailRequestBody", meta=(NativeBreakFunc))
