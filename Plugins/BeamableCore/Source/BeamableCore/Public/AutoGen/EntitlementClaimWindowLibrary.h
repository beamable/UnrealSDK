...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EntitlementClaimWindow To JSON String")
	static FString EntitlementClaimWindowToJsonString(const UEntitlementClaimWindow* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EntitlementClaimWindow", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEntitlementClaimWindow* Make(int64 Open, int64 Close, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EntitlementClaimWindow", meta=(NativeBreakFunc))
