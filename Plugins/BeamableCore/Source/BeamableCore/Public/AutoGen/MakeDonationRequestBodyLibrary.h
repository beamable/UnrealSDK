...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Groups|4 - Json", DisplayName="MakeDonationRequestBody To JSON String")
	static FString MakeDonationRequestBodyToJsonString(const UMakeDonationRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Groups|3 - Backend", DisplayName="Make MakeDonationRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="bAutoClaim, Outer", NativeMakeFunc))
	static UMakeDonationRequestBody* Make(int64 RecipientId, int64 Amount, FOptionalBool bAutoClaim, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Groups|3 - Backend", DisplayName="Break MakeDonationRequestBody", meta=(NativeBreakFunc))
