...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="IsoChronology To JSON String")
	static FString IsoChronologyToJsonString(const UIsoChronology* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make IsoChronology", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UIsoChronology* Make(FString CalendarType, FString Id, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break IsoChronology", meta=(NativeBreakFunc))
