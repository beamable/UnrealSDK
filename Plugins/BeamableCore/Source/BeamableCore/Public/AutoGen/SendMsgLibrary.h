...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Push|4 - Json", DisplayName="SendMsg To JSON String")
	static FString SendMsgToJsonString(const USendMsg* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Push|3 - Backend", DisplayName="Make SendMsg", meta=(DefaultToSelf="Outer", AdvancedDisplay="Notification, Data, Outer", NativeMakeFunc))
	static USendMsg* Make(TArray<int64> To, FOptionalSendNotification Notification, FOptionalMapOfString Data, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Push|3 - Backend", DisplayName="Break SendMsg", meta=(NativeBreakFunc))
