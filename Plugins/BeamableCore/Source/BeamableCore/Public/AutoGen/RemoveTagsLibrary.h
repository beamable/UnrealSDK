...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Lobby|4 - Json", DisplayName="RemoveTags To JSON String")
	static FString RemoveTagsToJsonString(const URemoveTags* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Lobby|3 - Backend", DisplayName="Make RemoveTags", meta=(DefaultToSelf="Outer", AdvancedDisplay="PlayerId, Tags, Outer", NativeMakeFunc))
	static URemoveTags* Make(FOptionalBeamGamerTag PlayerId, FOptionalArrayOfString Tags, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Lobby|3 - Backend", DisplayName="Break RemoveTags", meta=(NativeBreakFunc))
