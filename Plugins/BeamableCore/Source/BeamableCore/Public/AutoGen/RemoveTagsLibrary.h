...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="RemoveTags To JSON String")
	static FString RemoveTagsToJsonString(const URemoveTags* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make RemoveTags", meta=(DefaultToSelf="Outer", AdvancedDisplay="PlayerId, Tags, Outer", NativeMakeFunc))
	static URemoveTags* Make(FOptionalBeamGamerTag PlayerId, FOptionalArrayOfString Tags, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break RemoveTags", meta=(NativeBreakFunc))
