...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ListAuditResponse To JSON String")
	static FString ListAuditResponseToJsonString(const UListAuditResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ListAuditResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UListAuditResponse* Make(TArray<UPaymentAuditEntryViewModel*> Audits, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ListAuditResponse", meta=(NativeBreakFunc))
