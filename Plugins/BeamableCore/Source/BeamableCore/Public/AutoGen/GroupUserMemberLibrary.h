...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="GroupUserMember To JSON String")
	static FString GroupUserMemberToJsonString(const UGroupUserMember* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make GroupUserMember", meta=(DefaultToSelf="Outer", AdvancedDisplay="Joined, Outer", NativeMakeFunc))
	static UGroupUserMember* Make(int64 Id, TArray<UGroupUserMember*> SubGroups, FOptionalInt64 Joined, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break GroupUserMember", meta=(NativeBreakFunc))
