...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|4 - Json", DisplayName="EventPlayerPhaseView To JSON String")
	static FString EventPlayerPhaseViewToJsonString(const UEventPlayerPhaseView* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Make EventPlayerPhaseView", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEventPlayerPhaseView* Make(FString Name, int64 DurationSeconds, TArray<UEventRule*> Rules, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Break EventPlayerPhaseView", meta=(NativeBreakFunc))
