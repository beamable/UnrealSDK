...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Inventory|4 - Json", DisplayName="ItemContentResponse To JSON String")
	static FString ItemContentResponseToJsonString(const UItemContentResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Inventory|3 - Backend", DisplayName="Make ItemContentResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UItemContentResponse* Make(TArray<UItemArchetype*> Content, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Inventory|3 - Backend", DisplayName="Break ItemContentResponse", meta=(NativeBreakFunc))
