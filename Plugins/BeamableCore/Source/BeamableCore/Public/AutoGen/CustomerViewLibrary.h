...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Realms|4 - Json", DisplayName="CustomerView To JSON String")
	static FString CustomerViewToJsonString(const UCustomerView* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Make CustomerView", meta=(DefaultToSelf="Outer", AdvancedDisplay="Alias, Outer", NativeMakeFunc))
	static UCustomerView* Make(FBeamCid Cid, FString Name, TArray<UProjectView*> Projects, FOptionalString Alias, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Break CustomerView", meta=(NativeBreakFunc))
