...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Payments|4 - Json", DisplayName="GetProductsResponse To JSON String")
	static FString GetProductsResponseToJsonString(const UGetProductsResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Payments|3 - Backend", DisplayName="Make GetProductsResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGetProductsResponse* Make(TArray<UProductView*> Products, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Payments|3 - Backend", DisplayName="Break GetProductsResponse", meta=(NativeBreakFunc))
