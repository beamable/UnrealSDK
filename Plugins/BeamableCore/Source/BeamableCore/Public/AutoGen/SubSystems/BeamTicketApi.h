...
#include "BeamableCore/Public/AutoGen/SubSystems/Ticket/PostTicketsRequest.h"
#include "BeamableCore/Public/AutoGen/SubSystems/Ticket/GetTicketsRequest.h"
#include "BeamableCore/Public/AutoGen/SubSystems/Ticket/DeleteTicketsRequest.h"

#include "BeamTicketApi.generated.h"


/**
 * Subsystem containing request calls for the Ticket service.
 */
UCLASS(NotBlueprintType)
class BEAMABLECORE_API UBeamTicketApi : public UEngineSubsystem
{
private:
	GENERATED_BODY()
...
public:
	...
	static UBeamTicketApi* GetSelf() { return GEngine->GetEngineSubsystem<UBeamTicketApi>(); }

	

	
	/**
	 * @brief Makes an authenticated request to the Post /api/matchmaking/tickets endpoint of the Ticket Service.
	 *
	...
	/**
	 * @brief Makes an authenticated request to the Get /api/matchmaking/tickets/{id} endpoint of the Ticket Service.
	 *
	 * PREFER THE UFUNCTION OVERLOAD AS OPPOSED TO THIS. THIS MAINLY EXISTS TO ALLOW LAMBDA BINDING THE HANDLER.
	 * (Dynamic delegates do not allow for that so... we autogen this one to make experimenting in CPP a bit faster).
	 * 
	...
	/**
	 * @brief Makes an authenticated request to the Delete /api/matchmaking/tickets/{id} endpoint of the Ticket Service.
	 *
	 * PREFER THE UFUNCTION OVERLOAD AS OPPOSED TO THIS. THIS MAINLY EXISTS TO ALLOW LAMBDA BINDING THE HANDLER.
	 * (Dynamic delegates do not allow for that so... we autogen this one to make experimenting in CPP a bit faster).
	 * 
	 ...
	 * @param CallingContext A UObject managed by the UWorld that's making the request. Used to support multiple PIEs (see UBeamUserSlot::GetNamespacedSlotId) and read-only RequestCaches.
	 */
	UFUNCTION(BlueprintCallable, BlueprintInternalUseOnly, Category="Beam|Backend|Ticket", meta=(DefaultToSelf="CallingContext", AdvancedDisplay="OpHandle,CallingContext",AutoCreateRefTerm="UserSlot,OnSuccess,OnError,OnComplete,OpHandle", BeamFlowFunction))
