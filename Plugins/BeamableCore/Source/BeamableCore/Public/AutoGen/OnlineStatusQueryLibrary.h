...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="OnlineStatusQuery To JSON String")
	static FString OnlineStatusQueryToJsonString(const UOnlineStatusQuery* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make OnlineStatusQuery", meta=(DefaultToSelf="Outer", AdvancedDisplay="PlayerIds, Outer", NativeMakeFunc))
	static UOnlineStatusQuery* Make(FOptionalArrayOfString PlayerIds, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break OnlineStatusQuery", meta=(NativeBreakFunc))
