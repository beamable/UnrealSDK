...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Realms|4 - Json", DisplayName="RenameProjectRequestBody To JSON String")
	static FString RenameProjectRequestBodyToJsonString(const URenameProjectRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Make RenameProjectRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static URenameProjectRequestBody* Make(FBeamPid ProjectId, FString NewName, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Break RenameProjectRequestBody", meta=(NativeBreakFunc))
