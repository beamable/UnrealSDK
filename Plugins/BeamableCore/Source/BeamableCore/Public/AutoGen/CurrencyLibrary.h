...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Make Currency", meta=(DefaultToSelf="Outer", AdvancedDisplay="Properties, Outer", NativeMakeFunc))
	static UCurrency* Make(FString Id, int64 Amount, FOptionalArrayOfCurrencyProperty Properties, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Break Currency", meta=(NativeBreakFunc))
	static void Break(const UCurrency* Serializable, FString& Id, int64& Amount, FOptionalArrayOfCurrencyProperty& Properties);
};