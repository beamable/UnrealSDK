...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Tournaments|4 - Json", DisplayName="GetGroupsResponse To JSON String")
	static FString GetGroupsResponseToJsonString(const UGetGroupsResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Tournaments|3 - Backend", DisplayName="Make GetGroupsResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Focus, Outer", NativeMakeFunc))
	static UGetGroupsResponse* Make(TArray<UTournamentGroupEntry*> Entries, FOptionalTournamentGroupEntry Focus, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Tournaments|3 - Backend", DisplayName="Break GetGroupsResponse", meta=(NativeBreakFunc))
