...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="GroupSearchRequestBody To JSON String")
	static FString GroupSearchRequestBodyToJsonString(const UGroupSearchRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make GroupSearchRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="bHasSlots, bSubGroup, Name, ScoreMin, SortField, UserScore, EnrollmentTypes, Offset, ScoreMax, SortValue, Limit, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break GroupSearchRequestBody", meta=(NativeBreakFunc))
