...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Content|4 - Json", DisplayName="GetManifestsResponse To JSON String")
	static FString GetManifestsResponseToJsonString(const UGetManifestsResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Make GetManifestsResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGetManifestsResponse* Make(TArray<UContentBasicManifest*> Manifests, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Content|3 - Backend", DisplayName="Break GetManifestsResponse", meta=(NativeBreakFunc))
