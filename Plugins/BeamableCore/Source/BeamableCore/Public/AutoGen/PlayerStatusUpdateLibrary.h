...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="PlayerStatusUpdate To JSON String")
	static FString PlayerStatusUpdateToJsonString(const UPlayerStatusUpdate* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make PlayerStatusUpdate", meta=(DefaultToSelf="Outer", AdvancedDisplay="Tier, Stage, Score, Outer", NativeMakeFunc))
	static UPlayerStatusUpdate* Make(FOptionalInt32 Tier, FOptionalInt32 Stage, FOptionalDouble Score, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break PlayerStatusUpdate", meta=(NativeBreakFunc))
