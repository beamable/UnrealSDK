...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventPlayerGroupState To JSON String")
	static FString EventPlayerGroupStateToJsonString(const UEventPlayerGroupState* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventPlayerGroupState", meta=(DefaultToSelf="Outer", AdvancedDisplay="GroupId, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventPlayerGroupState", meta=(NativeBreakFunc))
