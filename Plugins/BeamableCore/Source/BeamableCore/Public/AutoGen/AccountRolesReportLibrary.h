...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="AccountRolesReport To JSON String")
	static FString AccountRolesReportToJsonString(const UAccountRolesReport* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make AccountRolesReport", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UAccountRolesReport* Make(int64 AccountId, FString Email, TArray<URealmRolesReport*> Realms, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break AccountRolesReport", meta=(NativeBreakFunc))
