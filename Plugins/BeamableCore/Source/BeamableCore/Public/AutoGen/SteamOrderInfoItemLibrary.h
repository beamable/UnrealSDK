...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="SteamOrderInfoItem To JSON String")
	static FString SteamOrderInfoItemToJsonString(const USteamOrderInfoItem* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make SteamOrderInfoItem", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static USteamOrderInfoItem* Make(int64 Vat, int64 Amount, int64 Itemid, int64 Qty, FString Itemstatus, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break SteamOrderInfoItem", meta=(NativeBreakFunc))
