...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Json", DisplayName="Promotion To JSON String")
	static FString PromotionToJsonString(const UPromotion* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Make/Break", DisplayName="Make Promotion", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UPromotion* Make(FString Id, UPromotable* Source, UPromotable* Destination, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Make/Break", DisplayName="Break Promotion", meta=(NativeBreakFunc))
