...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="Price To JSON String")
	static FString PriceToJsonString(const UPrice* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make Price", meta=(DefaultToSelf="Outer", AdvancedDisplay="Amount, Schedule, Outer", NativeMakeFunc))
	static UPrice* Make(FString Type, FString Symbol, FOptionalInt32 Amount, FOptionalArrayOfInt32 Schedule, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break Price", meta=(NativeBreakFunc))
