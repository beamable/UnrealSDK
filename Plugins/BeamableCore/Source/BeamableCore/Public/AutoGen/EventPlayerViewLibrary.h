...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|4 - Json", DisplayName="EventPlayerView To JSON String")
	static FString EventPlayerViewToJsonString(const UEventPlayerView* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Make EventPlayerView", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEventPlayerView* Make(TArray<UEventPlayerStateView*> Running, TArray<UEventPlayerStateView*> Done, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Break EventPlayerView", meta=(NativeBreakFunc))
