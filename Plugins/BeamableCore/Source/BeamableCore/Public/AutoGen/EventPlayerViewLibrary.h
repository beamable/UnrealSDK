...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="EventPlayerView To JSON String")
	static FString EventPlayerViewToJsonString(const UEventPlayerView* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make EventPlayerView", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UEventPlayerView* Make(TArray<UEventPlayerStateView*> Running, TArray<UEventPlayerStateView*> Done, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break EventPlayerView", meta=(NativeBreakFunc))
