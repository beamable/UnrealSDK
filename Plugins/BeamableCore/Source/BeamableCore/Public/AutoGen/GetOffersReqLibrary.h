...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Commerce|4 - Json", DisplayName="GetOffersReq To JSON String")
	static FString GetOffersReqToJsonString(const UGetOffersReq* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Make GetOffersReq", meta=(DefaultToSelf="Outer", AdvancedDisplay="Language, Time, Stores, Outer", NativeMakeFunc))
	static UGetOffersReq* Make(FOptionalString Language, FOptionalString Time, FOptionalString Stores, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Commerce|3 - Backend", DisplayName="Break GetOffersReq", meta=(NativeBreakFunc))
