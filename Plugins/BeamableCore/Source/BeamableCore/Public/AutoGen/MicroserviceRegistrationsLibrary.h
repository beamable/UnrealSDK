...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="MicroserviceRegistrations To JSON String")
	static FString MicroserviceRegistrationsToJsonString(const UMicroserviceRegistrations* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make MicroserviceRegistrations", meta=(DefaultToSelf="Outer", AdvancedDisplay="bTrafficFilterEnabled, RoutingKey, StartedById, BeamoName, Federation, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break MicroserviceRegistrations", meta=(NativeBreakFunc))
