...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CommerceLootRoll To JSON String")
	static FString CommerceLootRollToJsonString(const UCommerceLootRoll* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CommerceLootRoll", meta=(DefaultToSelf="Outer", AdvancedDisplay="ExternalTables, Outer", NativeMakeFunc))
	static UCommerceLootRoll* Make(bool bPreroll, FOptionalArrayOfString ExternalTables, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CommerceLootRoll", meta=(NativeBreakFunc))
