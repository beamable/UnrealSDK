...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="RealmConfigChangeRequestBody To JSON String")
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make RealmConfigChangeRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Deletes, Upserts, Outer", NativeMakeFunc))
	static URealmConfigChangeRequestBody* Make(FOptionalArrayOfString Deletes, FOptionalMapOfString Upserts, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break RealmConfigChangeRequestBody", meta=(NativeBreakFunc))
