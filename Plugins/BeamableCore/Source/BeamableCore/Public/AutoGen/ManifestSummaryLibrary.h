...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="ManifestSummary To JSON String")
	static FString ManifestSummaryToJsonString(const UManifestSummary* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make ManifestSummary", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UManifestSummary* Make(FString Uid, UContentBasicManifestChecksum* Manifest, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break ManifestSummary", meta=(NativeBreakFunc))
