...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="CalendarQueryResponse To JSON String")
	static FString CalendarQueryResponseToJsonString(const UCalendarQueryResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make CalendarQueryResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UCalendarQueryResponse* Make(TArray<UCalendarView*> Calendars, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break CalendarQueryResponse", meta=(NativeBreakFunc))
