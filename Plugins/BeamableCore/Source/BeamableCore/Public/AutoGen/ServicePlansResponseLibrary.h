...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Realms|4 - Json", DisplayName="ServicePlansResponse To JSON String")
	static FString ServicePlansResponseToJsonString(const UServicePlansResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Make ServicePlansResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UServicePlansResponse* Make(TArray<UServicePlan*> Result, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Realms|3 - Backend", DisplayName="Break ServicePlansResponse", meta=(NativeBreakFunc))
