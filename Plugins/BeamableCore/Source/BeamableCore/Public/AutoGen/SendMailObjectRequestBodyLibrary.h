...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Json", DisplayName="Beam - SendMailObjectRequestBody To JSON String")
	static FString SendMailObjectRequestBodyToJsonString(const USendMailObjectRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Make SendMailObjectRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Body, Expires, PlayerRewards, Subject, Rewards, Id, BodyRef, Attachments, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Break SendMailObjectRequestBody", meta=(NativeBreakFunc))
