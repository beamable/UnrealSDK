...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|4 - Json", DisplayName="ItemCreateRequestBody To JSON String")
	static FString ItemCreateRequestBodyToJsonString(const UItemCreateRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Make ItemCreateRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UItemCreateRequestBody* Make(FString ContentId, TArray<UItemProperty*> Properties, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|EventPlayers|3 - Backend", DisplayName="Break ItemCreateRequestBody", meta=(NativeBreakFunc))
