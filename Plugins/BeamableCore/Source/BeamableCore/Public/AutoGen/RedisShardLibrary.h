...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="RedisShard To JSON String")
	static FString RedisShardToJsonString(const URedisShard* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make RedisShard", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static URedisShard* Make(int32 ShardId, FString MasterHost, TArray<FString> SlaveHosts, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break RedisShard", meta=(NativeBreakFunc))
