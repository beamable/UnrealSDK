...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|4 - Json", DisplayName="GroupMetaData To JSON String")
	static FString GroupMetaDataToJsonString(const UGroupMetaData* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|3 - Backend", DisplayName="Make GroupMetaData", meta=(DefaultToSelf="Outer", AdvancedDisplay="Name, Tag, Outer", NativeMakeFunc))
	static UGroupMetaData* Make(int64 Id, FOptionalString Name, FOptionalString Tag, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|GroupUsers|3 - Backend", DisplayName="Break GroupMetaData", meta=(NativeBreakFunc))
