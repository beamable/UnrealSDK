...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="StatRequestBody To JSON String")
	static FString StatRequestBodyToJsonString(const UStatRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make StatRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Stats, Outer", NativeMakeFunc))
	static UStatRequestBody* Make(FOptionalString Stats, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break StatRequestBody", meta=(NativeBreakFunc))
