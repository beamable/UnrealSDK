...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Json", DisplayName="Beam - ActivationRequestBody To JSON String")
	static FString ActivationRequestBodyToJsonString(const UActivationRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Make ActivationRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UActivationRequestBody* Make(FString Token, int64 Cid, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Backend", DisplayName="Beam - Break ActivationRequestBody", meta=(NativeBreakFunc))
