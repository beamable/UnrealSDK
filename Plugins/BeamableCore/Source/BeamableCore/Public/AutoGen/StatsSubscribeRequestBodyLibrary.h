...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Stats|4 - Json", DisplayName="StatsSubscribeRequestBody To JSON String")
	static FString StatsSubscribeRequestBodyToJsonString(const UStatsSubscribeRequestBody* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Stats|3 - Backend", DisplayName="Make StatsSubscribeRequestBody", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UStatsSubscribeRequestBody* Make(FString Service, TArray<FString> Subscriptions, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Stats|3 - Backend", DisplayName="Break StatsSubscribeRequestBody", meta=(NativeBreakFunc))
