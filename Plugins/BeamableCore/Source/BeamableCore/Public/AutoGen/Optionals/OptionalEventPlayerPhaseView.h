
#pragma once

#include "CoreMinimal.h"
#include "Serialization/BeamOptional.h"
#include "AutoGen/EventPlayerPhaseView.h"

#include "OptionalEventPlayerPhaseView.generated.h"

// Has Native Make/Break require static blueprint pure functions to present as nodes that
// don't require an execution pin connection. This is super relevant for Blueprint UX. 
USTRUCT(BlueprintType, meta=(HasNativeMake="BeamableCore.OptionalEventPlayerPhaseViewLibrary.MakeOptionalEventPlayerPhaseView"))
struct FOptionalEventPlayerPhaseView : public FBeamOptional
{
	GENERATED_BODY()

	UPROPERTY(BlueprintReadOnly)
	UEventPlayerPhaseView* Val;

	FOptionalEventPlayerPhaseView();

	explicit FOptionalEventPlayerPhaseView(UEventPlayerPhaseView* Val);

	virtual const void* GetAddr() const override;

	virtual void Set(const void* Data) override;
};