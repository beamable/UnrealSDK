...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Json", DisplayName="AuthResponse To JSON String")
	static FString AuthResponseToJsonString(const UAuthResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Make/Break", DisplayName="Make AuthResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="AccessToken, RefreshToken, Outer", NativeMakeFunc))
	static UAuthResponse* Make(FOptionalString AccessToken, FOptionalString RefreshToken, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Customer|Utils|Make/Break", DisplayName="Break AuthResponse", meta=(NativeBreakFunc))
