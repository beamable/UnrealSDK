...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Auth|4 - Json", DisplayName="AuthResponse To JSON String")
	static FString AuthResponseToJsonString(const UAuthResponse* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Auth|3 - Backend", DisplayName="Make AuthResponse", meta=(DefaultToSelf="Outer", AdvancedDisplay="AccessToken, RefreshToken, Outer", NativeMakeFunc))
	static UAuthResponse* Make(FOptionalString AccessToken, FOptionalString RefreshToken, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Auth|3 - Backend", DisplayName="Break AuthResponse", meta=(NativeBreakFunc))
