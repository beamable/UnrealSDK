...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="Token To JSON String")
	static FString TokenToJsonString(const UToken* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make Token", meta=(DefaultToSelf="Outer", AdvancedDisplay="GamerTag, AccountId, Pid, ExpiresMs, Platform, Device, bRevoked, Scopes, Outer", NativeMakeFunc))
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break Token", meta=(NativeBreakFunc))
