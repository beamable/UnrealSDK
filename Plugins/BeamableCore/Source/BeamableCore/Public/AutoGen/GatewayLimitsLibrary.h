...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Shared|Json", DisplayName="GatewayLimits To JSON String")
	static FString GatewayLimitsToJsonString(const UGatewayLimits* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Make GatewayLimits", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UGatewayLimits* Make(int32 MaxConcurrentRequests, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|Shared|Backend", DisplayName="Break GatewayLimits", meta=(NativeBreakFunc))
