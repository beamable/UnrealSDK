...
#define LOCTEXT_NAMESPACE "K2BeamNode_Operation_TokenAuth"

UCLASS(meta=(BeamFlowNode))
class UK2BeamNode_Operation_TokenAuth : public UK2BeamNode_Operation
{
	GENERATED_BODY()

	virtual FText GetNodeTitle(ENodeTitleType::Type TitleType) const override { return LOCTEXT("Title", "Operation - Token Authentication"); }

	virtual FName GetSubsystemSelfFunctionName() const override { return GET_FUNCTION_NAME_CHECKED(UBeamRuntime, GetSelf); }

	virtual FName GetOperationFunctionName() const override { return GET_FUNCTION_NAME_CHECKED(UBeamRuntime, AuthenticateWithTokenOperation); }
