...
void BeamK2::ParseFunctionForNodePins(UEdGraphNode* CustomNode, const UFunction* Function, TArray<FString>& InputWrappedPinNames, TArray<FString>& OutputWrappedPinNames, const CheckParamIsValidForNodePredicate Predicate)
{
	...
	for (TFieldIterator<FProperty> PropIt(Function); PropIt && (PropIt->PropertyFlags & CPF_Parm); ++PropIt)
	{
		...
		if (Param->HasMetaData(*DynamicCastParameterName))
		{
			if (UPackage* Package = CustomNode->GetGraph()->GetOutermost())
			{
				if (UMetaData* MetaData = Package->GetMetaData())
				{
					if (MetaData->HasValue(CustomNode, CastTypeName))
					{
						auto CastType = MetaData->GetValue(CustomNode, CastTypeName);
