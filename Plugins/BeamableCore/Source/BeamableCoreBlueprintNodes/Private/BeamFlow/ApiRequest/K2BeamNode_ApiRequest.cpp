...
{
	return FText::FromString(FString::Printf(TEXT("Flow - API - %s - %s"), *GetServiceName(), *GetEndpointName()));
}

FText UK2BeamNode_ApiRequest::GetMenuCategory() const
{
	return FText::FromString(FString::Printf(TEXT("Beam|Flow|API|%s"), *GetServiceName()));
}
