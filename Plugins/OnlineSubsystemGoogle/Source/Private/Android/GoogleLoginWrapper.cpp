...
bool FGoogleLoginWrapper::Init(const FString& ServerClientId, bool bRequestServerAuthCode)
{
	if (JNIEnv* Env = FAndroidApplication::GetJavaEnv())
	{
		if (!GoogleLoginInstance)
		{
			...
			InitMethod = FJavaWrapper::FindMethod(Env, *GoogleLoginClass, "Init", "(Ljava/lang/String;Z)Z", false);
		...
		bool Ret = FJavaWrapper::CallBooleanMethod(Env, GoogleLoginInstance, InitMethod, *jServerClientId, jbRequestServerAuthCode);
		if (Env->ExceptionCheck())
		{
