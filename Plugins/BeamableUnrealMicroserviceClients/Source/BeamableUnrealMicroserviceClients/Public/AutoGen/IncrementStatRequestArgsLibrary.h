...
public:
	...
	UFUNCTION(BlueprintPure, Category="Beam|Json", DisplayName="IncrementStatRequestArgs To JSON String")
	static FString IncrementStatRequestArgsToJsonString(const UIncrementStatRequestArgs* Serializable, const bool Pretty);

	UFUNCTION(BlueprintPure, Category="Beam|2 - Backend", DisplayName="Make IncrementStatRequestArgs", meta=(DefaultToSelf="Outer", AdvancedDisplay="Outer", NativeMakeFunc))
	static UIncrementStatRequestArgs* Make(int64 UserID, UObject* Outer);

	UFUNCTION(BlueprintPure, Category="Beam|2 - Backend", DisplayName="Break IncrementStatRequestArgs", meta=(NativeBreakFunc))
